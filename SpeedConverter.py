...
def open_speed_conversion():
    speed_conv_path = abspath(join(dirname(__file__), 'SpeedConverter.py'))
    subprocess.Popen([sys.executable, speed_conv_path])


class SpeedConverter(ctk.CTk):
    def __init__(self):
        super().__init__()
        ...
        self.convert_button = ctk.CTkButton(self,
                                            text='Convert', font=('Arial', 14), command=self.convert_speed_unit)
