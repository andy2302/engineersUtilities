...
def open_pressure_converter():
    pressure_conv_path = abspath(join(dirname(__file__), 'PressureConverter.py'))
    subprocess.Popen([sys.executable, pressure_conv_path])


class PressureConverter(ctk.CTk):
    def __init__(self):
        super().__init__()
        ...
        units = {
            ...
            "Bars": 1,
            "Kilopascals": 0.01,
            "Millimeters of mercury": 0.001333223684,
            "Pascals": 0.00001,
            "Pounds per square inch": 0.0689476,
        }
        ...
        self.convert_button = ctk.CTkButton(self,
                                            text='Convert', font=('Arial', 14), command=self.convert_pressure_unit)
    ...
    def convert_pressure_unit(self):
        try:
            # get input values
