...
def open_area():
    area_path = abspath(join(dirname(__file__), 'AreaConverter.py'))
    subprocess.Popen([sys.executable, area_path])


class AreaConverter(ctk.CTk):
    def __init__(self):
        super().__init__()

        self.title('Area Converter')
        ...
        self.convert_button = ctk.CTkButton(self,
                                            text='Convert', font=('Arial', 14), command=self.convert_area)
